...
// Two functions:
//
//   gazelle_ts_ie_dispatch(req_ptr, req_len, *out_resp_ptr, *out_resp_len)
...
//     until gazelle_ts_ie_free.
//
//   gazelle_ts_ie_free(ptr, len)
//     Releases bytes returned by gazelle_ts_ie_dispatch.
...
/// `gazelle_ts_ie_free`.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn gazelle_ts_ie_dispatch(
    req_ptr: *const u8,
    req_len: usize,
...
/// # Safety
/// `ptr` must have been returned by `gazelle_ts_ie_dispatch` paired with the
/// same `len`. Calling with a null pointer is a no-op.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn gazelle_ts_ie_free(ptr: *mut u8, len: usize) {
...
mod tests {
    ...
    fn dispatch_roundtrip_via_ffi() {
        ...
        unsafe {
            gazelle_ts_ie_dispatch(req.as_ptr(), req.len(), &mut out_ptr, &mut out_len);
        }
        ...
        unsafe { gazelle_ts_ie_free(out_ptr, out_len) };
    }
    ...
    fn dispatch_empty_input_returns_error_response() {
        ...
        let mut out_len: usize = 0;
        unsafe {
            gazelle_ts_ie_dispatch(ptr::null(), 0, &mut out_ptr, &mut out_len);
        }
        assert!(!out_ptr.is_null());
        let resp_bytes = unsafe { slice::from_raw_parts(out_ptr, out_len) };
        let resp = pb::Response::decode(resp_bytes).expect("decode");
    ...
    #[test]
    fn gazelle_ts_ie_free_handles_null_and_zero() {
        unsafe { gazelle_ts_ie_free(ptr::null_mut(), 0) };
        unsafe { gazelle_ts_ie_free(ptr::null_mut(), 100) };
    }
}
